...
export class MongoDBScheduleRun implements IScheduleRunMemory {
	...
	public async updateScheduleRun(
		...
		return this.executeWithRetry(async () => {
			...
			const result = await ScheduleRunModel.updateOne(
				{ runId },
				{ $set: mutable },
			).maxTimeMS(timeout);
			if (result.matchedCount === 0) {
				throw new Error(`ScheduleRun not found: ${runId}`);
			}
		}, "updateScheduleRun()");
	}
	...
	public async listScheduleRuns(
		...
		return this.executeWithRetry(async () => {
			...
			// ?jobKey[$ne]=x into an operator object.
			if (typeof filter?.jobType === "string") query.jobType = filter.jobType;
			if (typeof filter?.jobKey === "string") query.jobKey = filter.jobKey;
