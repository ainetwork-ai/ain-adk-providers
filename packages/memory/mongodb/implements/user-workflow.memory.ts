...
export class MongoDBUserWorkflow implements IUserWorkflowMemory {
	...
	public async createUserWorkflow(
	...
	public async updateUserWorkflow(
		workflowId: string,
		...
		return this.executeWithRetry(async () => {
			const timeout = this.getOperationTimeout();
			const result = await UserWorkflowModel.updateOne(
				{ workflowId, userId },
				{ $set: mutableUpdates },
			).maxTimeMS(timeout);
	...
	public async deleteUserWorkflow(
		workflowId: string,
		userId: string,
	): Promise<void> {
		return this.executeWithRetry(async () => {
			const timeout = this.getOperationTimeout();
			await UserWorkflowModel.deleteOne({ workflowId, userId }).maxTimeMS(
				timeout,
			);
		}, "deleteUserWorkflow()");
	}
