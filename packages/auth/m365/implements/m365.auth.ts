...
export interface M365AuthConfig {
	...
	/**
	 * Exact `aud` values accepted for Azure AD tokens.
	 * Defaults to [clientId, `api://${clientId}`]. Providing this replaces the default list.
	 */
...
export class M365Auth extends AuthModule {
	...
	private readonly acceptedAudiences: [string, ...string[]];

	constructor(private readonly config: M365AuthConfig) {
		super();
		this.cloudInstance =
			this.config.cloudInstance || "https://login.microsoftonline.com";
		...
		// Strict audience allowlist: the app's client id and its api:// URI form.
		// An explicit acceptedAudiences config replaces the default list.
		this.acceptedAudiences = (
			this.config.acceptedAudiences?.length
				? this.config.acceptedAudiences
				: [this.config.clientId, `api://${this.config.clientId}`]
		) as [string, ...string[]];
		...
		return new Promise((resolve, reject) => {
			jwt.verify(
				...
				this.getSigningKey,
				{
					...
					// Strict audience enforcement. Do NOT fall back to appid/azp:
					// those identify the client app that acquired the token, not the
					// resource it was issued for, so accepting them enables cross-API
					// token replay.
	...
	public async authenticate(req: any, res: any): Promise<AuthResponse> {
		...
		try {
			...
			if (!payload.oid && !payload.sub) {
				console.error(
					"M365 auth verification failed: Token does not contain oid or sub claim",
				);
				return { isAuthenticated: false };
			}
